...
export default function PersonnelAppointmentsTable() {
  ...
  const handleCancelAppointment = async (
    row: Personnel["appointments"][number]
  ) => {
    const result = await Swal.fire({
      title: "¿Estás seguro?",
    ...
    if (result.isConfirmed) {
      try {
        ...
        if (data?.cancelAppointment.success) {
          Swal.fire("Éxito", "Has cancelado la cita");
          refetch();
        } else {
  ...
  const handleModalSubmit = async ({
    ...
    try {
      ...
      if (data?.completeAppointment?.success) {
        ...
        Swal.fire("Error", "Hubo un error al completar la cita.", "error");
      }
    } catch (error) {
          ...
          <MenuList>
            {row.status === "Pendiente" && (
              <>
                <MenuItem
                  onClick={() =>
                    handleAcceptAppointment(row.id, row.personnel.id)
                  }
                >
                  Confirmar cita
                </MenuItem>
                <MenuItem onClick={() => handleCancelAppointment(row)}>
                  Cancelar Cita
                </MenuItem>
              </>
            )}
            {row.status === "Confirmada" && (
              <MenuItem
                onClick={() =>
                  handleCompleteAppointment(row.id, row.personnel.id, row.date)
                } // Asegúrate de definir esta función
  ...
  const filteredAppointments =
    personnelData?.getPersonnelByRut?.appointments.filter(
      (appointment: Personnel["appointments"][number]) => {
